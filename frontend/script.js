...
function start() {
    ...
    uploadFile(replayData, ReplayFileDom.files[0], function(){
        if (ChartFileDom.value) {
            showChart = true;
            uploadFile(chartData, ChartFileDom.files[0], afterUpdate);
        } else {
            showChart = false;
            afterUpdate();
        }
...
/**
 * 通过 URL 直接加载回放文件（需要 HTTP 服务，file:// 协议下无效）
 * roadnetUrl: 路网文件路径，如 'web/roadnet_log.json'
 * replayUrl:  回放文件路径，如 'web/replay.txt'
 */
function startFromUrls(roadnetUrl, replayUrl) {
    if (loading) return;
    loading = true;
    infoReset();
    infoAppend("Auto-loading from server...");
    Promise.all([
        fetch(roadnetUrl).then(function(r) {
            if (!r.ok) throw new Error('roadnet: HTTP ' + r.status);
            return r.text();
        }),
        fetch(replayUrl).then(function(r) {
            if (!r.ok) throw new Error('replay: HTTP ' + r.status);
            return r.text();
        })
    ]).then(function(results) {
        roadnetData[0] = results[0];
        replayData[0]  = results[1];
        showChart = false;
        infoAppend("Files loaded: " + roadnetUrl + ", " + replayUrl);
        afterUpdate();
    }).catch(function(e) {
        infoAppend("Auto-load failed (" + e.message + "). Use HTTP server or upload files manually.");
        loading = false;
    });
}
