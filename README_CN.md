...
<div align="center">
  <img src="docs/image/frame_cn.png" alt="架构图" width="720">
</div>
...
| **完成通行车辆** | 已驶出路网的车辆数 | — |
| **仍在路网车辆** | 仿真结束时未驶出的车辆数 | — |
| **平均通行时间** | 车辆从生成到离开的平均时间（秒） | `sum(travel_times) / completed` |
| **平均延误** | 相比自由流多花的时间（秒） | `avg_travel_time − free_flow_time` |
| **平均排队车辆** | 每步等待车辆数的均值 | `sum(waiting_vehicles) / total_steps` |
| **最大排队车辆** | 仿真期间排队峰值 | `max(current_waiting)` |
| **车辆平均等待** | 每辆车处于等待状态的平均时长（秒） | `sum(vehicle_wait_steps) / total_spawned` |
| **平均停车次数** | 每辆完成通行车辆的平均停车次数 | `total_stops / completed` |
| **停车率** | 至少停过一次的车辆占比 | `stopped_vehicles / total_spawned × 100%` |
| **吞吐量** | 每秒完成通行的车辆数 | `completed / total_steps` |
| **相位切换次数** | 信号灯相位切换总次数（仅自适应模式） | — |
...
=================================================================
  ...
  完成通行车辆(↑)                  832        798    📈 +4.3%
  仍在路网车辆(↓)                   43         77    📈 +44.2%
  平均通行时间(↓)               35.2s      42.1s    📈 +16.4%
  平均延误(↓)                   17.2s      24.1s    📈 +28.6%
  平均排队车辆(↓)                 4.3        7.8    📈 +44.9%
  最大排队车辆(↓)                  15         28    📈 +46.4%
  车辆平均等待(↓)                8.5s      14.2s    📈 +40.1%
  平均停车次数(↓)                 1.2        2.1    📈 +42.9%
  停车率(↓)                     68.5%      82.3%   📈 +16.8%
  吞吐量(↑)                     0.231      0.222   📈 +4.1%
  相位切换次数                     52         18          —
  -----------------------------------------------------------------
  (↑)越大越好  (↓)越小越好  📈显著改善  📉显著退步  ➖基本持平
```

关键对比维度：
- **延误降低**：自适应 vs 固定配时的通行时间差异
- **排队管理**：平均排队越低，通行效率越高；最大排队反映峰值压力
- **停车次数**：越少停车，驾驶体验越好，油耗越低
- **吞吐量**：吞吐量越高，路口利用越充分

### 运行时指标（SDK）

SDK 额外通过 `get_metrics()` 跟踪运行时性能：
...
| `successful_decisions` | 返回有效相位的决策数 |
| `failed_decisions` | 决策失败数（返回 `None`） |
| `avg_inference_time_ms` | 每次决策平均推理耗时 |
